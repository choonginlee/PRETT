...
def three_handshake(dst_ip, dport, sport):
	...
	# RESPONSE = skt.sr1(ACK, verbose=False, retry=-1) # Send ACK, Listen http Response
	skt.send(ACK) # Send ACK
...
def generate_http_msg(payload, rp) :
	tcp_seg_len = get_tcp_seg_len(rp)
	p = IP(dst=dst_ip)/TCP(sport = sport, dport = dport, seq=rp.ack, ack=rp.seq+tcp_seg_len, flags = 'A')/(payload + ' HTTP/1.1' + delimiter + 'Host:' + str(dst_ip) + delimiter + delimiter)
	return p
...
def expand_states(states_in_the_level, token_db, args_db):
	...
	for current_state in states_in_the_level:
		...
		for msg in token_db: # group name
...
def process_response(p, ans, origin_rp):
	...
	"""
	There could be 3 cases of response in case of http.
